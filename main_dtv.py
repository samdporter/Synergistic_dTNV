...
def main(args):
    ...
    l21 = SmoothMixedL21Norm(epsilon=args.delta)
    grad = CompositionOperator(GradientOperator(ct))
    dir_op = DirectionalOperator(grad.direct(ct))
    dir_grad = CompositionOperator(dir_op, grad)
    tv = BlockFunction(OperatorCompositionFunction(l21, dir_grad), OperatorCompositionFunction(l21, dir_grad))
    prior = OperatorCompositionFunction(tv, bo)
